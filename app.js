...
const mysqlConnection = mysql.createConnection({
  ...
  user : 'u198684772_hbot',
  password : 'senhadobanco',
  database : 'u198684772_db'
});
...
if(!err)
  ...
  console.log('Falha na conexão com BD \n Error: ' + JSON.stringify(err, indefined, 2));
...
function updateMessage(input, response) {
  ...
  if (response.intents && response.intents[0]) {
    ...
    if (intent.intent = 'cardapio') {
        ...
        mysqlConnection.query('SELECT * FROM tb_usuarios', (err, rows, fields)=>{
            if(!err)
            response.output.text(rows);
            else
            console.log(err); 
        })
    };
